...
void ClusterGrowPLinkage::run( std::vector<std::vector<int> > &clusters, double &lambdaSTD, int &clusterNumber, int &finalplanenum)
{
	// create linkage
	std::vector<int> clusterCenterIdx;
	std::vector<std::vector<int> > singleLinkage;
    createLinkage( pcaInfos, clusterCenterIdx, singleLinkage, lambdaSTD);
    ...
    clusterNumber = clustersInit.size();

	// patch merging
	patchMerging( patchesInit, pcaInfos, clusters );
    finalplanenum = clusters.size();
}

void ClusterGrowPLinkage::createLinkage( std::vector<PCAInfo> &pcaInfos, std::vector<int> &clusterCenterIdx, std::vector<std::vector<int> > &singleLinkage, double &lambdaSTD )
{
    ...
    lambdaSTD = lambdaAvg + sqrt( lambdaVar / pcaInfos.size() );
